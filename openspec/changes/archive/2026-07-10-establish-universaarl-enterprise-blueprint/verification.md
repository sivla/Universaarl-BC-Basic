...
| Referenzen | Exit 0: 1 aktiver Change, nach Coherence-Korrektur 17 Domaenen/67 begruendete Unterfaehigkeiten sowie Faehigkeitsabhaengigkeiten, Jira-Hierarchie, Zyklen, Daten, Evidence-Status, Done-Gates und Confluence/OpenSpec-Referenzen konsistent |
| Gesamt | `npm test`, Exit 0 |
| Abhaengigkeiten | `npm audit`, Exit 0: `found 0 vulnerabilities`; direkte YAML-Pin-Version nach Audit auf 2.9.0 angehoben |
| Archivierungsprobe | `UABC-VER-ARCHIVE-DRYRUN-001`: in Wegwerfkopie Archive Exit 0, 13 Requirements in 3 Spec-Bereiche synchronisiert; danach `npm test` Exit 0 bei 0 aktiven Changes; stabile Architektur-, Faehigkeits- und Verifikationsartefakte vorhanden |
...
| Lieferkorrektur | `npm test`, `npm audit` und `git diff --check` bestanden nach W1-W5-Workstream-, Rollen-, Faehigkeits- und OQ-Synchronisierung; dies ist technische Konsistenz-Evidence und keine Faehigkeitsvalidierung |
...
Der erste Gesamtversuch war nicht gruen: OpenSpec verlangte das normative Schluesselwort `MUST` in allen ADDED Requirements. Nach gezielter Korrektur der Requirement-Saetze bestand der zweite Lauf. Das projektspezifische Schema selbst war in beiden Laeufen valide.

## Reproduzierbare lokale Checks

| Pruefung | Befehl | Akzeptanzkriterium |
| --- | --- | --- |
...
- Keine einzelne Faehigkeitsvalidierung, keine W1-Write-Erlaubnis und keine Go-live-Freigabe.
...
Status: **W0 bedingt freigegeben**. Umfang: Architektur, Faehigkeitenplan, Liefermodell, Rollen/Kontrollen und offene Gates. Ausgeschlossen: BC-/W1-Writes, `playthru`-Fakten, Rechts-/Steuerdetails, finale Lizenzierung, Faehigkeitsvalidierung und Go-live. Freigebender ist der reale Repository-Nutzer, nicht P-001.
