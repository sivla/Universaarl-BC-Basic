...
3. Drei sauber getrennte Confluence-Wissensraeume fuer Kundenprojekt, BC-Basic-Standardprodukt und internes Consultant-Handbuch mit eindeutiger Navigation, Entscheidungen, Daten, Schulung, Abrechnung und Uebergabe.
...
7. Ein `proposed` Projektindex und eine zugehoerige, fail-closed Project-Twin-Konsumentenbindung als einzige Einstiegskante. Der Twin loest den erlaubten Branch einmal auf, pinnt den vollstaendig validierten Commit und liest Index, Katalog und positivgelistete Blobs ausschliesslich aus diesem Commit; keine fachliche Wahrheit wird dupliziert.
8. Ein expliziter versionierter Uebergabevertrag in der Reihenfolge Spectra-Releasebindung im technischen BCProjectOS-Repository -> BC-Basic-Consumerbindung -> validierter Branch-Commit -> ausschliesslich lesender Project Twin. Historische Snapshotmanifeste bleiben Evidence, sind fuer den laufenden Branchvertrag aber nicht normativ.
...
13. Eine schlanke, dreigeteilte Confluence-Informationsarchitektur mit genau einer maschinenlesbaren Seitenbaumquelle: Kundenprojekt als alleinige Kundenwahrheit, kundenunabhaengiges BC-Basic-Produktbuch und internes Consultant-Handbuch. Die bestehenden 19 Seiten- und Story-IDs sowie ihre Pfade bleiben stabil; Titel, Parent-Beziehungen, Space-Zuordnung, Reihenfolge, modulare Navigation und Weiterleitung werden versioniert, formatgeprueft und fuer den Twin commitgebunden katalogisiert. Die bestehende Ticketstory liefert jedem Ticket seinen kanonischen Typ, eine validierte Parent-Typ-Beziehung und sichere Darstellungsmetadaten. Board, Kompaktliste, Spalten, Gruppen, Felder, Filter und Expand-Zustaende stammen aus dem Producervertrag; der Twin leitet nichts aus Key, Titel, Pfad oder CSS ab.
...
- **Dokumentdrift:** Der Twin darf Dokumenttypen, Phasen, Prozesse, Hierarchie, externe IDs oder URLs nicht heuristisch ableiten. Fehlende Blobs, Hashabweichungen, unsichere Pfade, doppelte IDs, defekte Referenzen, Zyklen, unzulaessige Dateitypen und nicht freigegebene externe Origins blockieren den gesamten Dokumentkatalog.
- **Vermischte Wissensraeume:** Kundenentscheidung, wiederverwendbarer Produktstandard und interne Beratungsmethode duerfen weder dupliziert noch als gleichartige Confluence-Seiten behandelt werden. Jede der 19 Seiten benoetigt genau einen internen Space, einen eindeutigen Parent und eine lesbare Zielgruppenansprache; externe Confluence-IDs und URLs bleiben ohne echte Quelle leer.
