...
const pages = [
  ['PAGE-UABC-000','UABC-PROJECT','00 Support',null,'UABC-SPACE-CUSTOMER',0,'atlassian/confluence/pages/00-project.md'],
  ...
  ['PAGE-UABC-030','UABC-BLUEPRINT','00 Consulting-Ueberblick',null,'UABC-SPACE-CONSULTANT',0,'atlassian/confluence/pages/30-blueprint.md'],
...
for(const page of pages){ if(newIds.has(page.storyPageId)){fs.mkdirSync('atlassian/confluence/pages',{recursive:true});fs.writeFileSync(page.sourcePath,newPageText(page),'utf8');}else updateExisting(page); }
...
index.governingChange='migrate-bc-basic-to-three-space-confluence-v1'; index.allowedBranch='codex/universaarl-projekt'; index.deliveryBranch='codex/bc-basic-three-space-v1';
...
story.pages=pages.map((page)=>({id:page.storyPageId,title:page.title,parent:page.parentId?pageIdByDocument.get(page.parentId):null,version:newIds.has(page.storyPageId)?1:(baseById.get(page.storyPageId)?.version??1)+1,status:'published',author_role:'project-team',time:'2026-07-12T12:00:00+02:00',sourcePath:page.sourcePath,references:referencesById.get(page.storyPageId),spaceId:page.spaceId,spaceType:spaceById.get(page.spaceId).spaceType,order:page.order}));
...
const contract={schemaVersion:1,contractId:'UABC-CONFLUENCE-THREE-SPACE-V1',projectId:'UABC-BC-BASIC-001',governingChange:index.governingChange,sourceMode:'repository-source-driven',deliveryBranch:'codex/bc-basic-three-space-v1',consumerProducerBranch:'codex/universaarl-projekt',spaceCount:3,rootDistribution:{customer:6,product:8,consultant:8},pageCount:28,legacyPageCount:19,newRequiredRootCount:9,truthBoundaries:{customer:'concrete-customer-source-of-truth',product:'pilot-working-state-what-is-sold',consulting:'pilot-working-state-how-it-is-delivered',spectra:'blueprint-candidate-only-no-automatic-adoption'},leadingContentOwnership:{customer:'concrete-instance-facts',product:'sold-and-delivered-scope',consulting:'delivery-method'},spaces,roots:pages.filter((page)=>page.parentId===null),children:pages.filter((page)=>page.parentId!==null),migrations,newRoots,forbiddenClaims:['live-atlassian','live-rovo','live-business-central','continia-execution','invented-approval','invented-spectra-release']};
