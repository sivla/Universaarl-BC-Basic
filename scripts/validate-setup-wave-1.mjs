...
export const PILOT_SETUP_BASELINE_PATH = 'project/bc-basic/pilot-setup-baseline.yaml';
...
export function validateSetupWave1({ matrix, runPlan, sources, sourceRegister, parameterBaseline, pilotSetupBaseline, coreSourceData, readOnlyPreflight, coreFinanceErrors = [] }) {
  ...
  if (configurationState.baselineClass !== 'standard-cronus-demo-baseline' || configurationState.baselineProvenance !== 'microsoft-standard-cronus-demo-data' || configurationState.appliedDifferenceStatus !== 'none-evidenced' || configurationState.configuredPilotClaimed !== false || configurationState.customerTargetRealized !== false || configurationState.originMechanismStatus !== 'unbekannt-bis-wave0-readback' || configurationState.copyRenameHypothesis !== 'nutzerhinweis-unbestaetigt' || configurationState.setupStatus !== 'blockiert-bis-dom-readback-und-zielkonfiguration' || configurationState.decisionStatus !== 'wave-0-read-only-preflight-required' || configurationState.companyStrategyStatus !== 'blocked-pending-wave0-and-reset-evidence' || configurationState.selectedCompanyStrategy !== null || configurationState.nextExecutableStep !== 'W0-01-read-company-identity' || configurationState.decisionContractPath !== `${PILOT_SETUP_BASELINE_PATH}#/companyInformation/companyStrategyDecision` || !/Standard-CRONUS/.test(configurationState.baselineTruth ?? '')) fail('CRONUS_BASELINE', 'Standard-CRONUS-Demo-Ausgangsbasis, unbekannte Gesellschaftsherkunft, Zielstrategie und Pilot-Soll muessen fail-closed getrennt sein');
  const companyInformation = pilotSetupBaseline?.companyInformation ?? {}; const strategy = companyInformation.companyStrategyDecision ?? {};
  if (companyInformation.currentState?.baselineKind !== 'standard-cronus-demo' || companyInformation.currentState?.baselineProvenance !== 'microsoft-standard-cronus-demo-data' || companyInformation.currentState?.customerTargetRealized !== false || companyInformation.currentState?.originMechanismStatus !== 'unbekannt-bis-wave0-readback' || companyInformation.currentState?.copyRenameHypothesis !== 'nutzerhinweis-unbestaetigt' || companyInformation.currentState?.setupStatus !== 'blockiert-bis-dom-readback-und-zielkonfiguration' || companyInformation.targetState?.classification !== 'bc-basic-target-not-applied' || companyInformation.appliedDifference?.status !== 'none-evidenced' || companyInformation.appliedDifference?.readbackEvidence?.length !== 0 || strategy.status !== 'blocked-pending-wave0-and-reset-evidence' || strategy.selectedOption !== null || strategy.allowedOptions?.join('|') !== 'controlled-reuse-of-dedicated-cronus-copy|clean-new-company-or-copy' || strategy.requiredEvidence?.length !== 6 || strategy.decisionEvidence?.length !== 0 || strategy.nextExecutableStep !== 'W0-01-read-company-identity' || strategy.writesAuthorized !== false) fail('COMPANY_STRATEGY_GATE', 'Zielstrategie muss bis zu vollstaendiger Wave-0-, Herkunfts- und Reset-Evidence unentschieden und schreibgesperrt bleiben');
  ...
  if (authorization?.status !== 'lesevorpruefung-freigegeben-schreibschritte-gesperrt' || authorization?.writesAuthorized !== false || authorization?.nextExecutableStep !== 'W0-01-read-company-identity' || authorization?.goReadOnlySteps?.join('|') !== expectedReadOnly.join('|') || authorization?.noGoWriteSteps?.join('|') !== expectedNoGo.join('|')) fail('AUSFUEHRUNGSGRENZE', 'nur W0-01 beziehungsweise PRE-01..12 und RUN-01..05 duerfen lesend freigegeben sein');
  ...
  if (baselineState.classification !== 'standard-cronus-demo-baseline' || baselineState.provenance !== 'microsoft-standard-cronus-demo-data' || baselineState.technicalCompanyName !== 'UABC-BASIC-DE' || baselineState.internalCompanyId !== 'unknown-required-before-write' || baselineState.configuredPilotClaimed !== false || pilotTarget.status !== 'pilot-build-pending' || pilotTarget.targetDecision !== 'pending-wave-0-evidence' || pilotTarget.companyStrategyStatus !== strategy.status || pilotTarget.decisionContractPath !== strategy.decisionAuthority || observedState.nextExecutableStep !== strategy.nextExecutableStep || appliedDifference.status !== 'none-evidenced' || appliedDifference.readbackEvidence?.length !== 0) fail('CRONUS_BASELINE', 'read-only Standard-CRONUS-Demo-Baseline, Pilot-Soll und angewendete Differenz sind nicht fail-closed getrennt');
  if (readOnlyPreflight?.companyStrategyDecision?.status !== strategy.status || readOnlyPreflight?.companyStrategyDecision?.selectedOption !== null || readOnlyPreflight?.companyStrategyDecision?.decisionEvidence?.length !== 0 || readOnlyPreflight?.companyStrategyDecision?.nextExecutableStep !== strategy.nextExecutableStep) fail('COMPANY_STRATEGY_GATE', 'Read-only Evidence darf keine unbelegte Zielstrategie auswaehlen');
  const wave0Checks = runPlan?.wave0Preflight?.checks ?? [];
  if (runPlan?.wave0Preflight?.attemptCount !== 2 || runPlan?.wave0Preflight?.latestAttemptId !== 'UABC-W0-01-ATTEMPT-002' || runPlan?.wave0Preflight?.latestAttemptAt !== '2026-07-13T14:11:07.3935478+02:00') fail('WAVE0_PREFLIGHT', 'Run-Plan muss beide blockierten W0-01-Versuche und den jüngsten Versuch eindeutig binden');
  if (runPlan?.wave0Preflight?.status !== 'blocked-before-dom-readback' || runPlan?.wave0Preflight?.currentBaseline !== 'standard-cronus-demo-baseline' || runPlan?.wave0Preflight?.configuredPilotClaimed !== false || runPlan?.wave0Preflight?.customerTargetRealized !== false || runPlan?.wave0Preflight?.originMechanismStatus !== 'unbekannt-bis-wave0-readback' || runPlan?.wave0Preflight?.copyRenameHypothesis !== 'nutzerhinweis-unbestaetigt' || runPlan?.wave0Preflight?.setupStatus !== 'blockiert-bis-dom-readback-und-zielkonfiguration' || runPlan?.wave0Preflight?.decisionContractPath !== strategy.decisionAuthority || runPlan?.wave0Preflight?.decisionGateStatus !== strategy.status || runPlan?.wave0Preflight?.nextExecutableStep !== strategy.nextExecutableStep || runPlan?.wave0Preflight?.requiredDecisionEvidence?.join('|') !== strategy.requiredEvidence?.join('|') || wave0Checks.map((item) => item.id).join('|') !== 'W0-01|W0-02|W0-03|W0-04|W0-05|W0-06' || wave0Checks[0]?.status !== 'blocked-before-dom-readback' || wave0Checks.slice(1).some((item) => item.status && item.status !== 'pending') || runPlan?.wave0Preflight?.selectedDecision !== null || runPlan?.wave0Preflight?.evidencePath !== 'evidence/playthru-uabc-basic-de/wave-0-company-identity-readback.yaml') fail('WAVE0_PREFLIGHT', 'Wave-0 muss den blockierten W0-01-Versuch, die unbekannte Gesellschaftsherkunft und den nicht realisierten Kundenstand source-driven binden und vor jedem Write offen bleiben');
