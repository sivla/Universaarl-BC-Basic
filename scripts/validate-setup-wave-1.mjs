...
export function validateSetupWave1({ matrix, runPlan, sources, sourceRegister, parameterBaseline, pilotSetupBaseline, coreSourceData, readOnlyPreflight }) {
  ...
  if (configurationState.baselineClass !== 'standard-cronus-demo-baseline' || configurationState.baselineProvenance !== 'microsoft-standard-cronus-demo-data' || configurationState.appliedDifferenceStatus !== 'none-evidenced' || configurationState.configuredPilotClaimed !== false || configurationState.decisionStatus !== 'wave-0-read-only-preflight-required' || configurationState.companyStrategyStatus !== 'blocked-pending-wave0-and-reset-evidence' || configurationState.selectedCompanyStrategy !== null || configurationState.nextExecutableStep !== 'W0-01-read-company-identity' || configurationState.decisionContractPath !== `${PILOT_SETUP_BASELINE_PATH}#/companyInformation/companyStrategyDecision` || !/Standard-CRONUS/.test(configurationState.baselineTruth ?? '')) fail('CRONUS_BASELINE', 'Standard-CRONUS-Demo-Ausgangsbasis, Zielstrategie und Pilot-Soll muessen fail-closed getrennt sein');
  const companyInformation = pilotSetupBaseline?.companyInformation ?? {}; const strategy = companyInformation.companyStrategyDecision ?? {};
  if (companyInformation.currentState?.baselineKind !== 'standard-cronus-demo' || companyInformation.currentState?.baselineProvenance !== 'microsoft-standard-cronus-demo-data' || companyInformation.targetState?.classification !== 'bc-basic-target-not-applied' || companyInformation.appliedDifference?.status !== 'none-evidenced' || companyInformation.appliedDifference?.readbackEvidence?.length !== 0 || strategy.status !== 'blocked-pending-wave0-and-reset-evidence' || strategy.selectedOption !== null || strategy.allowedOptions?.join('|') !== 'controlled-reuse-of-dedicated-cronus-copy|clean-new-company-or-copy' || strategy.requiredEvidence?.length !== 6 || strategy.decisionEvidence?.length !== 0 || strategy.nextExecutableStep !== 'W0-01-read-company-identity' || strategy.writesAuthorized !== false) fail('COMPANY_STRATEGY_GATE', 'Zielstrategie muss bis zu vollstaendiger Wave-0- und Reset-Evidence unentschieden und schreibgesperrt bleiben');
