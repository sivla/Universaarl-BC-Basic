...
const evidencePath = 'evidence/simulation/spectra-0.7-conformance.yaml';
...
if (errors.length) { console.error(`Spectra-0.7-Konformitaetspruefung fehlgeschlagen (${errors.length}):`); errors.forEach((error) => console.error(`- ${error}`)); process.exit(1); }
console.log(`Spectra-0.7-Konformitaetspruefung bestanden: 3 Angebotsstaende, 19 Seiten, 17 Tickets, 34 Kommentare, 17 Worklogs, 80h/9600 EUR, 15 Ereignisse, 3 Hypercaretage, 252 native Relationen, ${portable.graph.length} portable Kanten, Digest ${digest}.`);
