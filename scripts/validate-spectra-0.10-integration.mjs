...
export function validateIntegration(data) {
  ...
  if (native.length !== 252 || portable.length !== 348 || new Set(native.map((item) => item.id)).size !== 252 || new Set(portable.map((item) => item.id)).size !== 348) fail('COVERAGE_COUNTS', '252 native Relationen und 348 portable Kanten mit eindeutigen IDs erforderlich');
  ...
  if (conformance?.status !== 'passed' || conformance?.spectraRelease !== RELEASE.releaseTag || conformance?.portableConformance?.projectionDigest !== portableDigest || conformance?.adapterProvenance?.sourceHash !== sha256(data.indexBytes) || conformance?.adapterProvenance?.projectionDigest !== hash(data.exportMapBytes) || conformance?.referenceGraphCoverage?.sourceDigest !== sha256(data.coverageSourceBytes) || conformance?.referenceGraphCoverage?.mappingDigest !== sha256(data.coverageMappingBytes) || conformance?.referenceGraphCoverage?.projectionDigest !== sha256(data.coverageProjectionBytes) || conformance?.referenceGraphCoverage?.nativeRelations !== 252 || conformance?.referenceGraphCoverage?.portableEdges !== 348 || conformance?.referenceGraphCoverage?.oneToOneClaim !== false || conformance?.referenceGraphCoverage?.completeProjectionClaim !== false) fail('CONFORMANCE_EVIDENCE', '0.10-Konformitaetsevidence oder Digests weichen ab');
  ...
  if (story.offer?.versions?.length !== 3 || story.pages?.length !== 19 || story.tickets?.length !== 50 || story.tickets.filter((ticket) => ticket.type === 'task').length !== 19 || comments.length !== 38 || worklogs.length !== 19 || worklogs.reduce((sum, item) => sum + item.hours, 0) !== 80 || worklogs.reduce((sum, item) => sum + item.netAmount, 0) !== 9600 || story.timeline?.length !== 15 || story.hypercare?.length !== 3 || story.relations?.length !== 252) fail('STORY_COUNTS', '3/19/50 Tickets/19 Tasks/38/19/80/9600/15/3/252 erforderlich');
