...
export function validateIntegration(data) {
  ...
  const expectedConformanceState = { baselineKind: bcState.baselineKind, baselineProvenance: bcState.baselineProvenance, customerTargetRealized: bcState.customerTargetRealized, originMechanismStatus: bcState.originMechanismStatus, copyRenameHypothesis: bcState.copyRenameHypothesis, setupStatus: bcState.setupStatus, pilotConfigured: bcState.pilotConfigured, writesApplied: bcState.writesApplied, readbackStatus: bcState.readbackStatus, targetDecision: bcState.targetDecision, resetDecision: bcState.resetDecision, targetState: bcState.targetState, appliedDifferenceStatus: bcState.appliedDifferenceStatus, wave0AttemptStatus: wave0Attempt.status, wave0AttemptEvidence: wave0Attempt.evidencePath, wave0BcReadbackAuthority: wave0Attempt.bcReadbackAuthority, wave0BcFieldValuesRead: wave0Attempt.bcFieldValuesRead, wave0ScreenshotCaptured: wave0Attempt.screenshotCaptured, wave0WritesPerformed: wave0Attempt.writesPerformed, companyStrategyStatus: companyGate.status, selectedCompanyStrategy: companyGate.selectedOption, nextExecutableStep: companyGate.nextExecutableStep };
  if (bcState.baselineKind !== 'standard-cronus-demo' || bcState.baselineProvenance !== 'microsoft-standard-cronus-demo-data' || bcState.pilotConfigured !== false || bcState.writesApplied !== false || bcState.customerTargetRealized !== false || bcState.originMechanismStatus !== 'unbekannt-bis-wave0-readback' || bcState.copyRenameHypothesis !== 'nutzerhinweis-unbestaetigt' || bcState.setupStatus !== 'blockiert-bis-dom-readback-und-zielkonfiguration' || bcState.readbackStatus !== 'pending' || bcState.targetDecision !== 'pending-wave-0-evidence' || bcState.resetDecision !== 'pending-resetpoint-evidence' || bcState.targetState !== 'bc-basic-target-not-applied' || bcState.appliedDifferenceStatus !== 'none-evidenced' || wave0Attempt.status !== 'blocked-before-dom-readback' || wave0Attempt.evidencePath !== 'evidence/playthru-uabc-basic-de/wave-0-company-identity-readback.yaml' || wave0Attempt.bcReadbackAuthority !== false || wave0Attempt.bcFieldValuesRead !== false || wave0Attempt.screenshotCaptured !== false || wave0Attempt.writesPerformed !== false || companyGate.status !== 'blocked-pending-wave0-and-reset-evidence' || companyGate.selectedOption !== null || companyGate.nextExecutableStep !== 'W0-01-read-company-identity' || !equal(conformanceState, expectedConformanceState)) fail('BC_PILOT_STATE', 'Standard-CRONUS-Demo-Baseline, unbekannte Gesellschaftsherkunft, blockierter W0-01-Versuch, Pilot-Soll und Zielstrategie-Gate müssen strukturiert bis zur Konformitätsevidence reichen');
