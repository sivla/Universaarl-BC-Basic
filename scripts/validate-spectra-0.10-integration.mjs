...
export function validateIntegration(data) {
  ...
  if (baselineDiffers && (rec.variance?.reason_code === 'none' || !rec.variance?.reason?.trim() || !rec.variance.reason.includes('Spectra 0.10'))) fail('VARIANCE_REASON_REQUIRED', 'Baseline, Angebot und 0.10-Lieferstand sind nicht erklaert');
  ...
  if (native.length !== 252 || portable.length !== 328 || new Set(native.map((item) => item.id)).size !== 252 || new Set(portable.map((item) => item.id)).size !== 328) fail('COVERAGE_COUNTS', '252 native Relationen und 328 portable Kanten mit eindeutigen IDs erforderlich');
  ...
  if (coverage?.summary?.native_total !== 252 || coverage?.summary?.portable_edge_total !== 328 || coverage?.summary?.accounted_native_total !== 252 || coverage?.summary?.coverage_ratio !== 1 || coverage?.claims?.one_to_one_claim !== false || coverage?.claims?.complete_projection_claim !== false || coverage?.claims?.explanation_complete !== true) fail('COVERAGE_SUMMARY', 'Coverage erklaert nicht alle nativen Relationen ohne Vollstaendigkeitsbehauptung');
  ...
  if (conformance?.status !== 'passed' || conformance?.spectraRelease !== RELEASE.releaseTag || conformance?.portableConformance?.projectionDigest !== portableDigest || conformance?.adapterProvenance?.sourceHash !== sha256(data.indexBytes) || conformance?.adapterProvenance?.projectionDigest !== hash(data.exportMapBytes) || conformance?.referenceGraphCoverage?.sourceDigest !== sha256(data.coverageSourceBytes) || conformance?.referenceGraphCoverage?.mappingDigest !== sha256(data.coverageMappingBytes) || conformance?.referenceGraphCoverage?.projectionDigest !== sha256(data.coverageProjectionBytes) || conformance?.referenceGraphCoverage?.nativeRelations !== 252 || conformance?.referenceGraphCoverage?.portableEdges !== 328 || conformance?.referenceGraphCoverage?.oneToOneClaim !== false || conformance?.referenceGraphCoverage?.completeProjectionClaim !== false) fail('CONFORMANCE_EVIDENCE', '0.10-Konformitaetsevidence oder Digests weichen ab');
  ...
  if (currentTicketSurfaces.length !== 1 || currentTicketSurfaces[0]?.path !== TICKET_EXPORT_PATH || (data.index.artifacts ?? []).some((artifact) => artifact.kindId === 'jira-issues')) fail('TICKET_COUNTING_SURFACE', 'Nur der kanonische 86-Record-Ticketkatalog darf aktuelle Ticketansicht und Zaehlsurface sein');
  const ticketCatalog = data.index.ticketCatalog ?? {};
  if (ticketCatalog.path !== TICKET_EXPORT_PATH || ticketCatalog.sourceContract !== 'evidence/simulation/project-story.json' || ticketCatalog.recordCount !== 86 || ticketCatalog.customerStoryCount !== 48 || ticketCatalog.internalTraceabilityCount !== 38 || !equal(ticketCatalog.historicalSources, HISTORICAL_TICKET_SOURCES) || !equal(ticketCatalog.canonicalTypes, ['phase', 'epic', 'story', 'task', 'subtask', 'bug', 'change']) || ticketCatalog.typeField !== 'type' || ticketCatalog.canonicalTypeField !== 'canonicalType' || ticketCatalog.parentField !== 'parent' || ticketCatalog.visibilityRoleField !== 'visibilityRole' || ticketCatalog.countingScopeField !== 'countingScope' || ticketCatalog.typePresentationsField !== 'typePresentations' || ticketCatalog.typeLabelField !== 'typeLabel' || ticketCatalog.displayIconKeyField !== 'displayIconKey' || ticketCatalog.displayColorTokenField !== 'displayColorToken' || ticketCatalog.liveIconPolicyField !== 'liveIconPolicy' || ticketCatalog.viewsField !== 'views' || !equal(ticketCatalog.viewIds, TICKET_VIEWS.map((view) => view.id)) || !equal(ticketCatalog.viewTypes, TICKET_VIEWS.map((view) => view.type)) || !equal(ticketCatalog.presentationTypes, Object.keys(TICKET_TYPE_PRESENTATIONS)) || ticketCatalog.inferTypeFromKeyOrTitle !== false) fail('TICKET_EXPORT_VERTRAG', 'Branch-Index muss Tickettypen, Darstellungen, zwei Views und Zaehlscope explizit festlegen');
  ...
  if (story.offer?.versions?.length !== 3 || story.pages?.length !== 19 || story.tickets?.length !== 48 || story.tickets.filter((ticket) => ticket.type === 'task').length !== 19 || comments.length !== 38 || worklogs.length !== 19 || worklogs.reduce((sum, item) => sum + item.hours, 0) !== 80 || worklogs.reduce((sum, item) => sum + item.netAmount, 0) !== 9600 || story.timeline?.length !== 15 || story.hypercare?.length !== 3 || story.relations?.length !== 252) fail('STORY_COUNTS', '3/19/30 Tickets/17 Tasks/34/17/80/9600/15/3/252 erforderlich');
  ...
  for (const relative of [RECONCILIATION_PATH, PROVENANCE_PATH, MAP_PATH]) if (!(story.catalogs?.evidenceRefs ?? []).includes(relative)) fail('STORY_LINK_INCOMPLETE', relative);
