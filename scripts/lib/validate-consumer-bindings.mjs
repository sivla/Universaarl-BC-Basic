...
export function validateConsumerBindings(binding, projectIndex) {
  ...
  check(snapshot.pathSemantics === 'repository-relative' && snapshot.lifecycleStatus === 'proposed' && snapshot.digestAlgorithm === 'SHA-256' && snapshot.canonicalization === 'uabc-snapshot-records-v1', `${prefix}: Snapshotvertrag verwendet nicht den kanonischen Rohblobvertrag`);
  ...
  check(snapshot.manifestPath === null && snapshot.sourceCommitSha === null && snapshot.consumerBindingDigest === null && snapshot.payloadBundleDigest === null && snapshot.validationStatus === 'blocked', `${prefix}: aktueller Snapshot muss ohne Manifest und Nachweise blocked bleiben`);
  check(snapshot.availability === 'blockiert-bcprojectos-release-und-snapshotnachweise-ausstehend', `${prefix}: Snapshotavailability ist ungueltig`);
