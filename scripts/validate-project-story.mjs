...
export const validateStory = (story, { checkFiles = true, metadataReader = null } = {}) => {
  ...
  for (const page of pages) {
    ...
    // The historical story contains shared source documents; a newly introduced duplicate is always rejected.
    const historicalShared = new Set(['atlassian/confluence/pages/70-bc-basic-project.md','evidence/simulation/phase-2-p2p-o2c.yaml','evidence/simulation/phase-3-cash-inventory-close.yaml','project/bc-basic/uat-training-run.yaml']);
    if (pagePaths.has(page.sourcePath) && !historicalShared.has(page.sourcePath)) fail('SEITE-DOPPELTER-PFAD', page.sourcePath); pagePaths.add(page.sourcePath);
    ...
    if (metadataReader) { const meta = metadataReader(page.sourcePath); if (!meta || meta.id !== page.id || meta.parent !== page.parent || Number(meta.version) !== page.version || meta.status !== page.status) fail('SEITE-METADATEN-ABWEICHUNG', page.id); }
  }
  ...
  for (const ticket of tickets) {
    ...
    const history = ticket.statusHistory ?? []; const records = history.map((h) => typeof h === 'string' ? { status: h, time: ticket.createdAt } : h);
    if (!Array.isArray(history) || records.length < 2 || records.some((h) => !h || typeof h.status !== 'string' || !date(h.time)) || records.some((h,i) => i && h.time < records[i-1].time) || records[0]?.status !== 'created' || records.at(-1)?.status !== ticket.status || (records.some((h) => h.time < ticket.createdAt || h.time > ticket.closedAt))) fail('STATUSHISTORY-ABWEICHUNG', ticket.id);
    ...
    for (const worklog of ticket.worklogs ?? []) { ownOnly(worklog, WORKLOG_FIELDS, fail, `worklog[${ticket.id}]`); if (!date(worklog.date) || typeof worklog.role !== 'string' || typeof worklog.activity !== 'string' || typeof worklog.phase !== 'string' || typeof worklog.hours !== 'number' || worklog.hours <= 0) fail('WORKLOG-NESTED-TYP', ticket.id); totalHours += worklog.hours; totalCost += typeof worklog.cost === 'number' ? worklog.cost : worklog.hours * (story.controls?.worklogRate ?? 120); }
  }
  ...
  const timeline = story.timeline ?? []; const evidenceRefs = new Set([...story.readableSources ?? [], ...tickets.flatMap((t) => t.evidenceRefs ?? []), ...tickets.flatMap((t) => t.comments?.map((c) => c.evidenceRef) ?? []), 'SIM-HC-001','SIM-HC-003']); const sessions = new Set(['BC-SIM-SETUP-001','BC-SIM-MASTER-001','BC-SIM-P2P-001','BC-SIM-O2C-001','BC-SIM-CASH-001','BC-SIM-INVENTORY-001','BC-SIM-CLOSE-001']); const decisions = new Set(['beauftragt','GO','abgenommen','signoff','GO_SIMULATION','Exit','closed']); const deliverables = new Set(['OFR-UABC-BCB-001','UABC-26']);
  ...
  const inverse = { 'depends-on':'required-by', 'required-by':'depends-on', blocks:'blocked-by', 'blocked-by':'blocks', references:'references' }; const edgeSet = new Set();
  for (const relation of story.relations ?? []) { ownOnly(relation, RELATION_FIELDS, fail, `relation[${relation.from ?? '?'}]`); if (!inverse[relation.type]) fail('UNBEKANNTE-RELATION', relation.type); if (!ticketIds.has(relation.from) || !ticketIds.has(relation.to)) fail('RELATION-WAISE', relation.from); const key = `${relation.type}|${relation.from}|${relation.to}`; if (edgeSet.has(key)) fail('RELATION-DOPPELT', key); edgeSet.add(key); }
