...
export const validateStory = (story, { checkFiles = true, metadataReader = null } = {}) => {
  ...
  const wave0Worklog = wave0Ticket?.worklogs?.[0];
  if (wave0Ticket?.status !== 'blocked' || wave0Ticket?.worklogs?.length !== 1 || wave0Worklog?.id !== 'WL-UABC-39-W0-01-BLOCKED-20260713' || wave0Worklog?.hours !== 0.25 || wave0Worklog?.netAmount !== 30 || wave0Ticket?.evidenceRefs?.includes(wave0EvidencePath) !== true || wave0Ticket?.comments?.some((comment) => comment.type === 'closing')) fail('W0-01-TICKETWAHRHEIT', 'UABC-39 muss den blockierten Nur-Lese-Versuch mit 0,25 Stunden, Evidence und ohne Abschlusskommentar führen');
