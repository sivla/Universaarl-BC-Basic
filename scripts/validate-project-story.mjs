...
const validateCanonicalStory = (story, { checkFiles = true, metadataReader = null } = {}) => {
  const errors = []; const fail = (code, detail) => errors.push(`${code}: ${detail}`); const date = (value) => typeof value === 'string' && !Number.isNaN(Date.parse(value));
  if (story.classification !== 'synthetic-canonical-project-v1' || story.status !== 'simulated-complete') fail('STORY-STATUS', 'Kanonische Story muss synthetisch abgeschlossen sein');
  const truth = story.businessCentralPilotState ?? {};
  if (truth.status !== 'simulated-complete' || truth.pilotConfigured !== false || truth.writesApplied !== false || truth.customerTargetRealized !== false || truth.setupStatus !== 'synthetic-complete' || truth.readbackStatus !== 'synthetic-readback-complete-no-live-readback') fail('BC-SIMULATIONSSTATUS', 'Simulation und Realitaetsgrenze sind inkonsistent');
  if (story.controls?.currentAuthority !== true || story.controls?.realBcExecution !== false || story.controls?.simulationStatus !== 'simulated-complete' || story.controls?.realLiveGates !== 8) fail('SINGLE-TRUTH', 'Kanonische currentAuthority- und Realitaetsgrenze fehlt');
  if (story.offer?.planned_hours !== 80 || story.offer?.actual_hours !== 78 || story.offer?.planned_cost !== 9600 || story.offer?.actual_cost !== 9360 || story.activeOffer?.actualHours !== 78 || story.activeOffer?.actualNetAmount !== 9360) fail('WORKLOG-SUMME', 'Plan 80 Stunden/9.600 EUR und Ist 78 Stunden/9.360 EUR muessen konsistent sein');
  const pages = story.pages ?? []; const pageIds = new Set(pages.map((page) => page.id)); const pagePaths = new Set(); const metadata = metadataReader ?? readPageMetadata;
  if (pages.length !== 28) fail('SEITEN-ANZAHL', String(pages.length));
  for (const page of pages) { if (pagePaths.has(page.sourcePath)) fail('SEITE-DOPPELTER-PFAD', page.sourcePath); pagePaths.add(page.sourcePath); if (!page.id || !page.spaceId || !Number.isInteger(page.version) || !date(page.time) || !Array.isArray(page.references)) fail('SEITE-METADATEN-TYP', page.id); if (checkFiles && (!existsSync(page.sourcePath) || statSync(page.sourcePath).size === 0)) fail('SEITE-QUELLE-FEHLT', page.sourcePath); const meta = metadata(page.sourcePath); if (meta && (meta.id !== page.id || meta.title !== page.title || meta.parent !== page.parent || Number(meta.version) !== page.version || meta.status !== page.status || meta.spaceId !== page.spaceId || meta.spaceType !== page.spaceType || meta.order !== page.order)) fail('SEITE-METADATEN-ABWEICHUNG', page.id); if (page.parent !== null && !pageIds.has(page.parent)) fail('SEITE-WAISE', page.id); }
  const tickets = story.tickets ?? []; const ticketById = new Map(tickets.map((ticket) => [ticket.id, ticket])); const tasks = tickets.filter((ticket) => ticket.type === 'task'); const phases = tickets.filter((ticket) => ticket.type === 'phase'); const epics = tickets.filter((ticket) => ticket.type === 'epic'); const stories = tickets.filter((ticket) => ['story','bug'].includes(ticket.type));
  if (tickets.length !== 50 || phases.length !== 3 || epics.length !== 10 || stories.length !== 18 || tasks.length !== 19) fail('TICKET-HIERARCHIE', `${phases.length}/${epics.length}/${stories.length}/${tasks.length}`);
  const commentIds = new Set(); let hours = 0; let amount = 0;
  for (const ticket of tickets) { if (!['phase','epic','story','bug','task'].includes(ticket.type)) fail('TICKET-TYP', ticket.id); if (ticket.status !== 'closed' || !date(ticket.closedAt) || !date(ticket.testedAt) || !Array.isArray(ticket.statusHistory) || ticket.statusHistory.at(-1)?.status !== 'closed') fail('STATUSHISTORY-ABWEICHUNG', ticket.id); if (!Array.isArray(ticket.acceptanceCriteria) || ticket.acceptanceCriteria.length < 2 || ticket.acceptanceCriteria.some((item) => item.fulfilled !== true)) fail('ABNAHME-NICHT-ERFUELLT', ticket.id); if (!Array.isArray(ticket.comments) || !ticket.comments.some((item) => item.type === 'closing')) fail('ABSCHLUSS-KOMMENTAR-FEHLT', ticket.id); for (const comment of ticket.comments ?? []) { commentIds.add(comment.id); if (!comment.id || !date(comment.time) || !comment.evidenceRef) fail('KOMMENTAR-NESTED-TYP', ticket.id); } if (!Array.isArray(ticket.meetingTranscriptRefs) || ticket.meetingTranscriptRefs.length < 1) fail('MEETING-REFERENZ-FEHLT', ticket.id); if (!Array.isArray(ticket.evidenceRefs) || ticket.evidenceRefs.length < 1) fail('EVIDENCE-FEHLT', ticket.id); if (ticket.type === 'task') { if (!Array.isArray(ticket.worklogs) || ticket.worklogs.length !== 1 || ticket.billable !== true || ticket.hourlyRate !== 120) fail('WORKLOG-FEHLT', ticket.id); const taskHours = (ticket.worklogs ?? []).reduce((sum, item) => sum + Number(item.hours ?? 0), 0); const taskAmount = (ticket.worklogs ?? []).reduce((sum, item) => sum + Number(item.netAmount ?? 0), 0); if (ticket.actualHours !== taskHours || ticket.netAmount !== taskAmount) fail('TASK-ABRECHNUNG', ticket.id); hours += taskHours; amount += taskAmount; } else if ((ticket.worklogs ?? []).length !== 0) fail('ELTERN-WORKLOG', ticket.id); }
  if (hours !== 78 || amount !== 9360) fail('WORKLOG-SUMME', `${hours}/${amount}`);
  for (const ticket of tickets) { if (ticket.type === 'phase' && ticket.parent !== null) fail('PHASE-VERTRAG', ticket.id); if (ticket.type !== 'phase' && !ticketById.has(ticket.parent)) fail('TICKET-PARENT-TYP', ticket.id); const descendants = tasks.filter((task) => { let parent = task.parent; while (parent) { if (parent === ticket.id) return true; parent = ticketById.get(parent)?.parent ?? null; } return false; }); if (ticket.type !== 'task' && (ticket.actualHours !== descendants.reduce((sum, item) => sum + item.actualHours, 0) || ticket.netAmount !== descendants.reduce((sum, item) => sum + item.netAmount, 0))) fail('ROLLUP-ABWEICHUNG', ticket.id); }
  const sessions = new Set(story.catalogs?.sessions ?? []); const decisions = new Set(story.catalogs?.decisions ?? []); const evidence = new Set(tickets.flatMap((ticket) => [...(ticket.evidenceRefs ?? []), ...(ticket.comments ?? []).map((comment) => comment.evidenceRef)])); for (const event of story.timeline ?? []) { for (const id of event.tickets ?? []) if (!ticketById.has(id)) fail('UNBEKANNTE-TIMELINE-REFERENZ', id); for (const id of event.pages ?? []) if (!pageIds.has(id)) fail('UNBEKANNTE-TIMELINE-REFERENZ', id); for (const id of event.sessions ?? []) if (!sessions.has(id)) fail('UNBEKANNTE-TIMELINE-REFERENZ', id); if (event.evidence && !evidence.has(event.evidence)) fail('UNBEKANNTE-TIMELINE-REFERENZ', event.evidence); if (event.result !== 'synthetisch-abgenommen') fail('TIMELINE-STATUS', event.id); if (!date(event.time)) fail('TIMELINE-NESTED-TYP', event.id); }
  for (const day of story.hypercare ?? []) if (day.status !== 'synthetic-complete' || day.retest !== 'bestanden-synthetisch' || !commentIds.has(day.comment) || !decisions.has(day.decision)) fail('HYPERCARE-RECORD', String(day.day));
  return errors;
};

export const validateStory = (story, { checkFiles = true, metadataReader = null } = {}) => {
  if (story.status === 'simulated-complete' || story.classification === 'synthetic-canonical-project-v1') return validateCanonicalStory(story, { checkFiles, metadataReader });
  ...
  const wave0Attempt = bcState.wave0ReadbackAttempt ?? {};
  const wave0EvidencePath = 'evidence/playthru-uabc-basic-de/wave-0-company-identity-readback.yaml';
  if (bcState.baselineKind !== 'standard-cronus-demo' || bcState.baselineProvenance !== 'microsoft-standard-cronus-demo-data' || bcState.pilotConfigured !== false || bcState.writesApplied !== false || bcState.customerTargetRealized !== false || bcState.originMechanismStatus !== 'unbekannt-bis-wave0-readback' || bcState.copyRenameHypothesis !== 'nutzerhinweis-unbestaetigt' || bcState.setupStatus !== 'blockiert-bis-dom-readback-und-zielkonfiguration' || bcState.readbackStatus !== 'pending' || bcState.technicalCompanyName !== 'UABC-BASIC-DE' || bcState.internalCompanyId !== null || bcState.targetDecision !== 'pending-wave-0-evidence' || bcState.resetDecision !== 'pending-resetpoint-evidence' || bcState.targetState !== 'bc-basic-target-not-applied' || bcState.appliedDifferenceStatus !== 'none-evidenced' || wave0Attempt.status !== 'blocked-before-dom-readback' || wave0Attempt.evidencePath !== wave0EvidencePath || wave0Attempt.bcReadbackAuthority !== false || wave0Attempt.bcFieldValuesRead !== false || wave0Attempt.screenshotCaptured !== false || wave0Attempt.writesPerformed !== false || companyGate.status !== 'blocked-pending-wave0-and-reset-evidence' || companyGate.selectedOption !== null || companyGate.nextExecutableStep !== 'W0-01-read-company-identity' || companyGate.authority !== 'project/bc-basic/pilot-setup-baseline.yaml#/companyInformation/companyStrategyDecision' || bcState.sourceEvidence !== wave0EvidencePath || bcState.planningEvidence !== 'evidence/playthru-uabc-basic-de/setup-wave-1-read-only-preflight.yaml' || (bcState.observedDisplayName === bcState.targetDisplayName && bcState.targetDisplayName)) fail('BC-PILOT-ZUSTAND', 'Standard-CRONUS-Demo-Baseline, Nutzerinformation, unbekannte Gesellschaftsherkunft, blockierter W0-01-Versuch, Pilot-Soll und Zielstrategie-Gate müssen strukturiert getrennt bleiben');
  ...
  if (!['current-pilot-planning', 'synthetic-canonical-project-v1'].includes(story.classification) || !['in-progress', 'simulated-complete'].includes(story.status)) fail('STORY-STATUS', 'Kanonischer Simulationsstatus ist ungueltig');
  ...
  if (story.status === 'simulated-complete' ? (story.offer?.currentStatus !== 'synthetic-closed' || story.activeOffer?.status !== 'synthetic-closed' || story.activeOffer?.customerAcceptanceClaimed !== false || story.activeOffer?.syntheticAcceptanceClaimed !== true) : (story.offer?.currentVersion !== 'pilot-rebaseline-2026-07-13' || story.offer?.currentStatus !== 'active-planning' || story.activeOffer?.status !== 'planned-not-accepted' || story.activeOffer?.customerAcceptanceClaimed !== false)) fail('ANGEBOT-STATUS', 'Angebotsstatus ist nicht konsistent');
  ...
  if (wave0Attempt.attemptCount !== 2 || wave0Attempt.latestAttemptId !== 'UABC-W0-01-ATTEMPT-002' || wave0Attempt.latestAttemptAt !== '2026-07-13T14:11:07.3935478+02:00') fail('BC-PILOT-ZUSTAND', 'Die Projektstory muss beide blockierten W0-01-Versuche und den jüngsten Versuch eindeutig binden');
  const wave0Ticket = ticketById.get('UABC-39');
  const wave0Worklogs = wave0Ticket?.worklogs ?? [];
  if (wave0Ticket?.status !== 'blocked' || wave0Worklogs.length !== 2 || wave0Worklogs.map((worklog) => worklog.id).join('|') !== 'WL-UABC-39-W0-01-BLOCKED-20260713|WL-UABC-39-W0-01-RETRY-BLOCKED-20260713' || wave0Worklogs.some((worklog) => worklog.hours !== 0.25 || worklog.netAmount !== 30) || wave0Ticket?.actualHours !== 0.5 || wave0Ticket?.netAmount !== 60 || wave0Ticket?.comments?.filter((comment) => comment.evidenceRef === wave0EvidencePath).length !== 3 || wave0Ticket?.evidenceRefs?.includes(wave0EvidencePath) !== true || wave0Ticket?.comments?.some((comment) => comment.type === 'closing')) fail('W0-01-TICKETWAHRHEIT', 'UABC-39 muss beide blockierten Nur-Lese-Versuche mit 0,50 Stunden, drei Arbeitskommentaren, Evidence und ohne Abschlusskommentar führen');
  ...
  for (const ticket of tickets) {
    ...
    if (typeof ticket.summary !== 'string' || ticket.summary.trim().length < 8 || ticket.summary.length > 60 || typeof ticket.title !== 'string' || ticket.title !== ticket.summary) fail('TICKET-TITEL', `${ticket.id}: Titel muss prägnant, 8 bis 60 Zeichen lang und identisch zur Summary sein`);
    if (typeof ticket.description !== 'string' || ticket.description.length < 80 || /Status im aktuellen Playthru-Pilot|Aktueller Playthru-Pilot:|soll geprüft werdener|historischn/i.test(`${ticket.summary} ${ticket.description}`) || (OPEN_TICKET_STATUSES.has(ticket.status) && /wurden entschieden|wurden geladen|wurde durchgespielt|Retest bestanden|als Handover abgeschlossen|synthetisch abgeschlossen|erfolgreich durchgeführt|erfolgreich abgeschlossen/i.test(ticket.description))) fail('TICKET-TEXT', ticket.id);
