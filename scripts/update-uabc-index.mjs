...
index.ticketCatalog={path:'atlassian/jira/issues/bc-basic-story-tickets.yaml',sourceContract:'evidence/simulation/project-story.json',migrationMatrix:'project/bc-basic/ticket-migration.yaml',recordCount:50,customerStoryCount:50,internalTraceabilityCount:0,canonicalTypes:['phase','epic','story','task'],typeField:'type',canonicalTypeField:'canonicalType',parentField:'parent',visibilityRoleField:'visibilityRole',countingScopeField:'countingScope',typePresentationsField:'typePresentations',typeLabelField:'typeLabel',displayIconKeyField:'displayIconKey',displayColorTokenField:'displayColorToken',liveIconPolicyField:'liveIconPolicy',viewsField:'views',presentationTypes:['phase','epic','story','task'],viewIds:['UABC-TICKET-VIEW-BOARD-001','UABC-TICKET-VIEW-COMPACT-001'],viewTypes:['board','compact-list'],inferTypeFromKeyOrTitle:false};
index.referenceDefinitions=index.referenceDefinitions||{}; index.referenceDefinitions.jiraRefs=Array.from({length:50},(_,i)=>`UABC-${i+1}`);
index.documentCatalog=index.documentCatalog||{}; index.documentCatalog.referenceDefinitions=index.documentCatalog.referenceDefinitions||{}; index.documentCatalog.referenceDefinitions.jiraRefs=Array.from({length:50},(_,i)=>`UABC-${i+1}`);
index.consumerRules=(index.consumerRules||[]).map(r=>typeof r==='string'&&r.includes('drei Phase-Tickets')?'Ausschliesslich ticketCatalog.path ist die aktuelle Ticketansicht und Zaehlsurface: drei Phase-Tickets, zehn fachliche Epics, 18 Stories und 19 abrechenbare Tasks; nur Task-Worklogs zaehlen 80 Stunden und 9.600 EUR.':r);
...
for (const definition of index.documentCatalog?.definitions ?? []) {
  ...
  definition.jiraRefs = metadata.jiraRefs;
}
if(index.documentCatalog) index.documentCatalog.documentCount=34;
fs.writeFileSync(path,YAML.stringify(index)); console.log('Branch-Index auf aktiven UABC-50-Vertrag aktualisiert.');
