...
test('kanonische Simulation bleibt synthetisch abgeschlossen', () => { assert.deepEqual(validateActiveSimulation(story, runPlan, projection), []); assert.deepEqual(validateActiveBcPlaythrough(story, runPlan, projection), []); assert.equal(story.offer.actual_hours, 80); assert.equal(story.offer.actual_cost, 9600); });
