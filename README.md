...
Aktiv ist `deliver-bc-basic-customer-project`. Der Produktvertrag heisst fachlich Spectra (`productId: spectra`); die technische Repository-Identitaet bleibt BCProjectOS. Spectra `0.9.0-alpha.1` ist ueber den annotierten Tag `spectra-v0.9.0-alpha.1` und reproduzierbare Evidence gebunden. Die native Kundenstory bleibt alleinige Source of Truth. Der veroeffentlichte Baseline-Angebot-Ist-Vertrag und die read-only Adapter-Provenienz werden auf die vorhandene synthetische Story und den einzigen Branch-Index angewendet; die Twin-Exportmap ist eine deterministische Projektion und keine zweite Projektwahrheit. Der Project Twin liest spaeter ausschliesslich den neuesten vollstaendig validierten Commit des festgelegten BC-Basic-Branches. Historische A/B-Evidence bleibt unveraendert, ist aber keine laufende Pflichtstrecke.
