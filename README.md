...
Aktiv ist `deliver-bc-basic-customer-project`. Der Produktvertrag heisst fachlich Spectra (`productId: spectra`); die technische Repository-Identitaet bleibt BCProjectOS. Spectra `0.6.0-alpha.1` ist ueber den annotierten Tag `spectra-v0.6.0-alpha.1` und reproduzierbare Evidence gebunden. Der Project Twin liest spaeter ausschliesslich den neuesten vollstaendig validierten Commit des festgelegten BC-Basic-Branches; Payload, Index und Integritaetsangaben werden gemeinsam committet. Historische A/B-Evidence bleibt unveraendert, ist aber keine laufende Pflichtstrecke. `establish-project-artifact-walkthrough-pilot` bleibt archivierte Historie.
