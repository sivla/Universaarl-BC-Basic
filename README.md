...
Aktiv ist `deliver-bc-basic-customer-project`. Der Produktvertrag heisst fachlich Spectra (`productId: spectra`); die technische Repository-Identitaet bleibt BCProjectOS. Spectra `0.7.0-alpha.1` ist ueber den annotierten Tag `spectra-v0.7.0-alpha.1` und reproduzierbare Evidence gebunden. Die native Kundenstory bleibt alleinige Source of Truth; eine temporaere deterministische Projektion ermoeglicht die portable Spectra-Full-Conformance. Der Project Twin liest spaeter ausschliesslich den neuesten vollstaendig validierten Commit des festgelegten BC-Basic-Branches. Historische A/B-Evidence bleibt unveraendert, ist aber keine laufende Pflichtstrecke.
