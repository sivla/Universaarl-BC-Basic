# Walkthrough-Paket verfassen

1. Kopiere das Blanko und ersetze jeden Platzhalter. Danach muss die Kopie `npm run validate:walkthrough-manifest -- <datei>` bestehen. Blanko, Beispiel und erzeugte Darstellung sind laut `evidenceSemantics` abgeleitete Lern-/Darstellungsartefakte und selbst kein fachlicher Nachweis. Referenzierte Quellnachweise bleiben echte Provenienz.
...
6. Lege Rohvideo, Trace, Auth und Tempdateien nie in das Paket. Der Generator referenziert nur ihre Lauf- und Nachweisprovenienz.
7. `npm run validate:walkthrough-manifest -- <datei>` prueft nur den generischen Draft-2020-12-Vertrag. `npm run build:walkthrough:baseline` baut ausschliesslich den fest definierten Baseline-Piloten `UABC-WT-ENV-001` und prueft zusaetzlich dessen sieben `ENV-*`-Schritte, `run-1`/`run-2`, Nachweise und Medienregeln.
