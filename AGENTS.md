...
- **Aktueller Koordinationsstatus:** Die kanonische Repository-Identitaet von BCProjectOS ist `https://github.com/sivla/BCProjectOS.git`. Spectra `1.0.0` ist ueber den echten Release-Tag `spectra-v1.0.0`, den verifizierten Tag-Commit `c05649bd10ed29a082bbe2338d7d326d3d755687`, das finale Manifest und den Payload-Digest in der lokalen Release-Evidence nachgewiesen. Der Blueprint-Agent richtet weder Remote noch Branch, Upstream, Tag oder Release fuer BCProjectOS ein.
- Der aktuelle Twin-Vertrag ist ein Git-unabhaengiger Kundenkatalog: `exports/project-data/v1/snapshots/current.json` zeigt atomar auf genau ein unveraenderliches Release mit Manifest, Index, Ressourcenkatalog und Payloadbytes. Eine Producer-Commit-SHA darf nur Provenienz sein und ist keine Runtimevoraussetzung. Historische Portable-Evidence bleibt als Historie erhalten.
...
- Nur diese Kundeninstanz darf den Project-Twin-Katalog erzeugen. Ein freigegebener Katalog nennt mindestens Schema-Version, Kunden-/Projekt-ID, Spectra-Bindung, Digest, Validierungsstatus sowie sichere relative Artefaktpfade und Selektoren; Git ist fuer das Lesen nicht erforderlich.
