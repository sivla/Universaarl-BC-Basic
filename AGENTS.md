...
- **Aktueller Koordinationsstatus:** Die kanonische Repository-Identitaet von BCProjectOS ist `https://github.com/sivla/BCProjectOS.git`. Spectra `0.8.0-alpha.1` ist ueber den annotierten Tag `spectra-v0.8.0-alpha.1`, den Tag-Commit `5dd23c2ff2c408c03fd613348fcb305635cfbf9a`, das finale Manifest und den reproduzierbaren Payload-Digest in `evidence/spectra-release-0.8.0-alpha.1.yaml` commitgebunden nachgewiesen. Der Blueprint-Agent richtet weder Remote noch Branch, Upstream, Tag oder Release fuer BCProjectOS ein.
